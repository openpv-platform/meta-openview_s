...
# We're going to source a file that is going to define the environment variables containing the command line options for
# the mount to allow us to change whether it is going to mount read-only or not.
MOUNT_PARTITIONS_SOURCE_FILE="/etc/mount-opts.sh"
if [ -f "$MOUNT_PARTITIONS_SOURCE_FILE" ]; then
  source "$MOUNT_PARTITIONS_SOURCE_FILE"
fi
...
case "$1" in
    start)
        ...
        do
            ...
            [ -e $DEVICE ] && mount $DEVICE_OPTION $(eval "echo \$${part_label}_OPTIONS") $DEVICE $mountpoint
        done
        ;;
    stop)
        # umount partitions
