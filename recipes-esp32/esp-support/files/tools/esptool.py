...
try:
    ...
    print("gpiod is not installed for %s." % (sys.executable))
    raise
...
ESP_GPIO_CHIP = 'gpiochip2'
ESP_IO12_CHIP = 'gpiochip3'
ESP_IO0_GPIO_PIN = 11                 # PC11 = WLAN_BOOT_NORM, low at startup = flash enable
ESP_EN_GPIO_PIN = 15                  # PC15 = WLAN_PWR_EN
ESP_IO12_GPIO_PIN = 5                 # PD5 = dual use sdmmc3/io12 held low at startup for 3.3v flash
ESP_IO2_GPIO_PIN = 1                  # PD1 = IO2 needs to be low for boot loader, don't care for normal boot

# Set up GPIO pins
chip = GPIO.chip(ESP_GPIO_CHIP)
en_pin = chip.get_line(ESP_EN_GPIO_PIN)
io0_pin = chip.get_line(ESP_IO0_GPIO_PIN)
config = GPIO.line_request()
config.consumer = "esp32"
config.request_type = GPIO.line_request.DIRECTION_OUTPUT
en_pin.request(config)
io0_pin.request(config)
chip = GPIO.chip(ESP_IO12_CHIP)
io12_pin = chip.get_line(ESP_IO12_GPIO_PIN)
io2_pin = chip.get_line(ESP_IO2_GPIO_PIN)
io12_pin.request(config)
io2_pin.request(config)
# Always set io12 low for our purposes
io12_pin.set_value(0)
io2_pin.set_value(0)

# Release the pins - I think this ensures that the kernel or other devices can then reclaim them
def release_pins():
    en_pin.release()
    io0_pin.release()
    io2_pin.release()
...
class ESPLoader(object):
    ...
    DEFAULT_PORT = "/dev/ttySTM2"
    # Not sure of this value - Document I have shows 0x3ff44038
    GPIO_STRAP_REG = 0x3ff44038 # 0x3f404038
    GPIO_STRAP_REG_ALT = 0x3f404038
    ...
    def __init__(self, port=DEFAULT_PORT, baud=ESP_ROM_BAUD, trace_enabled=False):
        ...
        # lines = chip.get_lines([ESP_EN_GPIO_PIN, ESP_IO0_GPIO_PIN])
        # lines.request(config)
        # chip.get_line(ESP_EN_GPIO_PIN)
        # self.en_pin = lines[0]
        # chip.get_line(ESP_IO0_GPIO_PIN)
        # self.io0_pin = lines[1]
    ...
    def _setDTR(self, state):
        # DTR is active low, we are reversing desired state
        val = 0 if state else 1
        io0_pin.set_value(val)
        # io0_pin.set_value(state)

    def _setRTS(self, state):
        # RTS is active low, we are reversing desired state
        val = 0 if state else 1
        en_pin.set_value(val)
...
def write_flash(esp, args):
    ...
    # Try to read the strapping register
    strap_reg = esp.read_reg(esp.GPIO_STRAP_REG)
    print("Strapping Register: {:#x}"
            .format(strap_reg))
